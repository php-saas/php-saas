@if ($subscription = user()->subscription())
    @php
        $planArray = collect(config('billing.plans'))
            ...
            ->first();
        $plan = $planArray ? \App\DTOs\BillingPlanDTO::fromArray($planArray) : null;
    @endphp
        ...
        <x-ui.card class="w-full">
            <x-ui.card.header>
                <div class="flex items-start justify-between">
                    <div>
                        <x-ui.card-title>{{ $plan->getName() }}</x-ui.card-title>
                        <x-ui.card-description>Current Plan</x-ui.card-description>
                    </div>
                    <div class="flex items-center">
                        <span class="font-semibold">${{ $plan->getPrice() }}</span>
                        @if ($plan->getPriceId())
                            <span class="text-muted-foreground font-semibold">/{{ $plan->getBilling() }}</span>
                        @endif
                    </div>
                </div>
            </x-ui.card.header>
    ...
    <x-ui.card class="w-full">
        <x-ui.card.header>
            ...
            <x-ui.card-description>{{ __('Here you can manage your subscription') }}</x-ui.card-description>
        </x-ui.card.header>
